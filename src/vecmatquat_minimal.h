...
public:
    ...
    float3x3(float3 x, float3 y, float3 z)
        : x(x)
        , y(y)
        , z(z)
    {
...
inline float4 QuatFromAxisAngle(const float3& axis, float t)
{
    auto v = normalize(axis) * sinf(t / 2.0f);
    return { v.x, v.y, v.z, cosf(t / 2.0f) };
}
inline std::pair<float3, float> AxisAngleFromQuat(const float4& q)
{
    auto a = acos(q.w) * 2.0f;
    return std::make_pair(q.xyz() / sinf(a / 2.0f), a);
}
